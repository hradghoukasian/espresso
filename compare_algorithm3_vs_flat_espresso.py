...
"""
Experiment: compare Algorithm 1 (sample-based multi-stage influence-residual
learning on a partial truth table) with flat Espresso on the ambient
B-dimensional partial truth table.

Optimization:
- Algorithm 1 caches each learned projected stage as a complete truth table.
- Later residual computations use fast list lookups instead of repeatedly
  evaluating PyEDA expressions.
- The one-time caching cost is included in Algorithm 1's reported training time.
...
class EspressoModel:
    ...
    Algorithm 1 uses cached truth tables.
...
def train_algorithm3(
    ...
    for t in range(1, stages_m + 1):
        ...
        if model.tt_full is None:
            raise RuntimeError("Expected a cached truth table for Algorithm 1.")
...
def run_experiment(args: argparse.Namespace) -> Path:
    ...
    for stage in stage_points:
        ...
        summary_rows.append(
            {
                "method": f"Algorithm 1 (stage {stage})",
                "B": args.B,
                "S-junta": args.S,
                "K": args.K,
                "train_size": args.train_size,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[1, 5, 20],
        help="stages at which Algorithm 1 accuracy/runtime are reported",
    )
    parser.add_argument(
        "--num-seeds",
        type=int,
